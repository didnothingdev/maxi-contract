...
export class BaseSpl {
  ...
  async createToken(opts: CreateTokenOptions) {
    this.__reinit();
    let { decimals: decimal, payer, freezeAuthority, mintKeypair, initFeePercentage, maxFeeTokens } = opts;
    ...
    freezeAuthority = /* freezeAuthority ?? mintAuthority */ null;
    decimal = decimal ?? 0;
    mintKeypair = mintKeypair ?? web3.Keypair.generate();
    initFeePercentage = initFeePercentage ?? 20_00;
    maxFeeTokens = maxFeeTokens ?? BigInt(100000_000000);
    const mint = mintKeypair.publicKey;
    // console.log('  MINT_SIZE:', MINT_SIZE);
    const mintLen = getMintLen(extensions);
    // console.log('  mintLen:', mintLen);
    const rent = await this.connection
      .getMinimumBalanceForRentExemption(/* MINT_SIZE */ mintLen)
      .catch(async () => {
        await sleep(2_000);
    ...
    const ix1 = web3.SystemProgram.createAccount({
      ...
      space: /* MINT_SIZE */ mintLen,
    });
    this.splIxs.push(ix1);
    
    const ix2 = createInitializeTransferFeeConfigInstruction(
  ...
  createTokenAccount(
    ...
    const ata = getAssociatedTokenAddressSync(mint, owner, allowOffCurveOwner, TOKEN_2022_PROGRAM_ID);
  ...
  async transfer(input: TransferInput) {
    ...
    const fee = amount / 5; // 20%
    const senderAta = getAssociatedTokenAddressSync(mint, sender, true, TOKEN_2022_PROGRAM_ID);
    const receiverAta = getAssociatedTokenAddressSync(
      mint,
      receiver,
      receiverIsOffCurve,
  ...
  async getAllSplMints(owner: web3.PublicKey) {
    const tokenAccounts = await this.connection
      .getTokenAccountsByOwner(owner, { programId: TOKEN_2022_PROGRAM_ID })
